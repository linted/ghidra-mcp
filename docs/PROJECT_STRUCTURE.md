...
├── python/ghidra_mcp_bridge/    # Python MCP bridge package (`ghidra-mcp-bridge`)
├── pyproject.toml / uv.lock     # Python packaging + locked deps (uv)
├── pom.xml                      # Canonical Maven build
├── build.gradle                 # Secondary/manual Gradle path
├── requirements*.txt            # Python dependencies (requirements.txt generated from pyproject)
...
| Run the MCP bridge | `uv run ghidra-mcp-bridge` |
